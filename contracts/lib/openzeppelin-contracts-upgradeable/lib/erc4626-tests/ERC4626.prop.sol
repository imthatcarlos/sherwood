...
abstract contract ERC4626Prop is Test {
    ...
    function prop_convertToShares(address caller1, address caller2, uint256 assets) public {
        vm.prank(caller1); // "MAY revert due to integer overflow caused by an unreasonably large input."
        uint256 res1 = vault_convertToShares(assets);
        vm.prank(caller2); // "MAY revert due to integer overflow caused by an unreasonably large input."
        uint256 res2 = vault_convertToShares(assets);
        assertEq(res1, res2);
    }
    ...
    function prop_convertToAssets(address caller1, address caller2, uint256 shares) public {
        vm.prank(caller1); // "MAY revert due to integer overflow caused by an unreasonably large input."
        uint256 res1 = vault_convertToAssets(shares);
        vm.prank(caller2); // "MAY revert due to integer overflow caused by an unreasonably large input."
        uint256 res2 = vault_convertToAssets(shares);
        assertEq(res1, res2);
    }
    ...
    function prop_previewDeposit(address caller, address receiver, address other, uint256 assets) public {
        vm.prank(other); // "MAY revert due to other conditions that would also cause deposit to revert."
        uint256 sharesPreview = vault_previewDeposit(assets);
        vm.prank(caller);
    ...
    function prop_previewWithdraw(address caller, address receiver, address owner, address other, uint256 assets)
        public
    {
        vm.prank(other);
